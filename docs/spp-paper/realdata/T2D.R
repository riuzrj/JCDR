...
T2D_functional = read.csv("/Users/ruijuanzhong/JCDR/docs/spp-paper/realdata/T2D/T2D_functional.csv")
T2D_genomic = read.csv("/Users/ruijuanzhong/JCDR/docs/spp-paper/realdata/T2D/T2D_genomic40.csv")
T2D_taxonomic_species = read.csv("/Users/ruijuanzhong/JCDR/docs/spp-paper/realdata/T2D/T2D_taxonomic_species.csv")
# 修改列名，添加后缀 ".KEGG"
colnames(T2D_genomic) <- paste(colnames(T2D_genomic), "KEGG", sep = ".")

#合并数据
T2D <- rbind(T2D_functional, T2D_genomic, T2D_taxonomic_species)
X <- t(T2D)
X <- as.data.frame(X)
dim(X)
#读取Y标签
T2D_y <- read.csv("/Users/ruijuanzhong/project/microbiome/T2D/T2D_ylab.txt", header = F)
Y = factor(T2D_y$V1)
length(Y)
Y = as.factor(Y)
table(Y)
...
algs <- list(lol.project.pca, lol.project.lrlda, lol.project.lol, lol.project.pls,
...
for (i in 1:length(algs)) {
  ...
  tryCatch({
    ...
    results <- rbind(results, data.frame(sim='sun', fold=xv_res$folds.data$fold, alg=names(algs)[i],
...
results <- t(resultso)
write.csv(results, file=paste('/Users/ruijuanzhong/JCDR/', 'T2D.csv', sep=""), row.names=FALSE)
# filter out bad rows
...
filterd_results <- results %>%
  ...
  #把大于 30 的 r 值过滤掉
  filter(r <= 50)
  #将 sim 名 nakayama 改为 burczynski
  #mutate(sim = ifelse(sim == 'nakayama', 'burczynski', sim))
...
smoothed_results <- smoothed_results %>%
  group_by(sim, alg) %>%
  summarize(x = list(desired_r), y = list(spline(r, lhat, xout = desired_r, method = 'natural', ties = mean)$y)) %>%
  unnest(c(x, y))
...
dt.star <- dt_plot %>% filter(x == r.star)
dt_plot %>%
  # slice(nan.idx) %>%
  ggplot(aes(x=x, y=y, color=alg)) +
