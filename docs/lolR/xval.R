...
#' This function produces fold-wise cross-validated misclassification rates for standard embedding techniques. 
...
#' Optional classifiers implementing the S3 \code{predict} function can be used for classification,
#'  with hyperparameters to classifiers for determining misclassification rate specified in \code{classifier.*} parameters and hyperparameters.
#'
#' @importFrom MASS lda
...
#'
#' A function for performing leave-one-out cross-validation for a given embedding model,
#' that allows users to determine the optimal number of embedding dimensions for their algorithm-of-choice.
#' This function produces fold-wise cross-validated misclassification rates for standard embedding techniques
#'  across a specified selection of embedding dimensions. Optimal embedding dimension is selected
#'  as the dimension with the lowest average misclassification rate across all folds.
#' Users can specify custom embedding techniques with proper configuration of \code{alg.*} parameters and hyperparameters.
#' Optional classifiers implementing the S3 \code{predict} function can be used for classification,
#'  with hyperparameters to classifiers for determining misclassification rate specified in \code{classifier.*}.
#'
#' @importFrom MASS lda
#' @importFrom stats predict
...
lol.xval.split <- function(X, Y, k='loo', rank.low=FALSE, ...) {
    ...
    if (round(k) == k) {  # then xval is an integer
        samp.idx <- as.matrix(sample(1:n, n))  # the sample idx randomly permuted
        k.folds <- split(samp.idx, rep(1:k), drop=TRUE)  # split the sample ids into xval folds
...
# Self-defined funcs.
classifier.meas <- function(y,yhat) {
    y <- as.factor(y)
    cm <- caret::confusionMatrix(yhat,y)
    accuracy <- cm$overall[1]
    precision <- posPredValue(y_hat, test_y, positive="1")
    recall <- sensitivity(y_hat, test_y, positive="1")
...
lol.xval.meas <- function(X, Y, rs, alg, sets=NULL, alg.dimname="r", alg.opts=list(), alg.embedding="A",
                                       alg.structured=TRUE, classifier=lda, classifier.opts=list(),
                                       classifier.return="class", k=10, rank.low=FALSE, ...) {
    d <- dim(X)[2]
    Y <- factor(Y)
    n <- length(Y)
    x.n <- dim(X)[1]
    ...
    # check that if the user specifies the cross-validation set, if so, that it is correctly set up, otherwise, do it.
    if (is.null(sets)) {
        sets <- lol.xval.split(X, Y, k=k, rank.low=rank.low)
    }
    else {
